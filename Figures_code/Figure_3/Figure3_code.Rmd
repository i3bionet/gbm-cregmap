...
# recurrenece
rec_gbm_tb <- vroom("../gbm_EDA/recurrent/all248samples_GBM_cRegMap_class.txt")
```
...
png(filename = "Figure_3/Fig3A_complex_heatmap.png",
      height   = 20.3,
      width    = 9,
      units    = 'in',
      res      = 300)
...
ggsave(plot     = ck_GO2_p,
       device   = "png",
       filename = "Figure_3/Fig3B_gbm_compare_GO_k7.png",
       units    = "in",
       width    = 10,
       height   = 8,
       dpi      = 400)
...
file_folder <- "Figure_3"

png(filename =  file.path(file_folder, "Fig3C_GBM_all_clusters_oncoplot.png"),
...
for (group in unique(MutationalData@clinical.data$seuratClusters)){
    ...
    png(filename = file.path(file_folder, paste0("Fig3C_GBM_", group, "_hr_genes.png")), 
        width = 1800, height = 2000, res = 300)
...
# h_gene_set_OXPHO <- h_gene_sets %>%  filter(str_detect(gs_name, "OXIDATIVE_PHOSPHORYLATION"))


rankData <- singscore::rankGenes(expTcga1)
# list("HIF1_signature" = HIF1_GS, "AMPK_signature" = AMPK))
genes_intersect <- 
  map(c(h_gene_set, GO_gene_set), 
        ~intersect(rownames(expTcga1), unique(.x$gene_symbol)))
#genes_int_OXPHO <- intersect(rownames(expTcga1), unique(h_gene_set_OXPHO$gene_symbol))
scores_agilent <- map(genes_intersect,
                      ~simpleScore(rankData,  upSet = .x) %>% 
  as_tibble(rownames = "sample")) %>% 
  bind_rows(.id = "Hallmark")

#scores_OXPHO  <-  simpleScore(rankData, upSet = genes_int_OXPHO) 
                    
### save terms  ------
wb <- createWorkbook()
addWorksheet(wb, "S1")
writeDataTable(wb, "S1", x = scores_agilent, withFilter = FALSE)
saveWorkbook(wb, file.path("tcga_agilent_gbm_score_hallmarks_GOBP.xlsx"),
             overwrite = TRUE)
...
# draw(ht_1, heatmap_legend_side="right", 
#      annotation_legend_side="right",
#            legend_grouping = "original")
################################################################################ 
##add the signscore heatmap
...
#rowv <- sign_score %>% dplyr::select(-sample) %>% names
...
png(filename = "Figure_3/Fig3D_GBM_signscore_deconv.png",
      height   = 14,
      width    = 10,
      units    = 'in',
      res      = 300)
...
ggsave(plot = purity_plot,
         device = "png",
         filename = file.path("Figure_3/Fig3E_purity_plot_tcga.png"),
         units = "in",
         width = 8,
         height = 16,
         dpi = 300)
...
png(filename = "Figure_3/Fig3F_gbmMetacohort_k7_survCurve.png", 
...
webshot::webshot(url = html_tmp,
                 file = "Figure_3/Fig3G_sankeyPlot_k7_recurrent.png",
                 delay = 0.2)
```
